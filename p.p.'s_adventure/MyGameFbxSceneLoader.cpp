...
void MyGameFbxSceneLoader::getNodeFromFbxNode(MyGameSceneManager *sceneMgr,
                                              FbxNode *node,
                                              MyGameSceneNode *parentSceneNode,
                                              MyGameSceneContainer &container,
                                              const string& prefix)
{
    MyGameSceneNode* snode = sceneMgr->CreateSceneNode( string( prefix + node->GetName()).c_str() );
    FbxDouble3 translation = node->LclTranslation.Get();
    FbxDouble3 rotation = node->LclRotation.Get();
    FbxDouble3 scaling = node->LclScaling.Get();
    snode->setPosition( translation.mData[0], translation.mData[1], translation.mData[2] );
    ...
    for( int i = 0; i != node->GetNodeAttributeCount(); ++ i )
    {
        ...
        if( node->GetNodeAttributeByIndex( i )->GetAttributeType() == FbxNodeAttribute::eMesh )
        {
            ...
            mesh->loadMeshFromFbxNodeAttribute( node->GetNodeAttributeByIndex( i ) );
            stringstream ss;
            ss<< prefix << node->GetName() << i;
            //MyGameSceneEntity* ent = sceneMgr->CreateSceneEntity( mesh, ss.str().c_str() );
            MyGameSceneEntity* ent = sceneMgr->CreateSceneEntity( mesh, snode );
            //snode->attachEntity( ent );
    ...
    for( int i = 0; i != node->GetChildCount(); ++ i )
    {
        getNodeFromFbxNode( sceneMgr, node->GetChild( i ), snode, container, prefix );
    }
}

void MyGameFbxSceneLoader::loadSceneFromFbx(MyGameSceneManager *sceneMgr, const string &fileName, const string& prefix, MyGameSceneContainer& container)
{
    FbxManager* fbxMgr= FbxManager::Create();
    FbxIOSettings* fbxIO = FbxIOSettings::Create( fbxMgr, IOSROOT );
    fbxMgr->SetIOSettings( fbxIO );
    ...
    getNodeFromFbxNode( sceneMgr, rootNode, container.getRootNode(), container, prefix );
